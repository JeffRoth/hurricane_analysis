...
def convert_damages_to_float(damages):
    ...
    for damage in damages:
        if damage == "Damages not recorded":
            damages_float.append("Damages not recorded")
        if "M" in damage:
            damages_float.append( float(damage.strip("M")) * conversion["M"] )
        if "B" in damage:
            damages_float.append( float(damage.strip("B")) * conversion["B"] )
...
def construct_hurricane_dictionary(names, months, years, max_sustained_winds, areas_affected, damages_float, deaths):
    ...
    i = 0
    for name in names:
        hurricane_dictionary[name] = {"Name": names[i],
                                    "Month": months[i],
                                    "Year": years[i],
                                    "Max Sustained Wind": max_sustained_winds[i],
                                    "Areas Affected": areas_affected[i],
                                    "Damage": damages_float[i],
                                    "Deaths": deaths[i]}
        i += 1
...
def hurricane_dictionary_by_year(hurricane_dictionary):
    ...
    for cane in hurricane_dictionary:
        ...
        current_cane = hurricane_dictionary[cane]
...
def most_affected_area(areas_count):
    ...
    max_key = max(areas_count, key=areas_count.get)
...
def rate_mortality(hurricanes):
    ...
    # initialize dictionary with list as values
    for i in range(5):
        hurricane_mortalities[i]=[]
    ...
    for cane in hurricanes:
        ...
        for i in mortality_scale:
            if deaths <= mortality_scale[i]:
                values = hurricane_mortalities[i]
                values.append(hurricanes[cane])
                hurricane_mortalities[i] = values
                break
...
def greatest_damage(hurricanes):
    ...
    for cane in hurricanes:
        ...
        # deal with non-numeric damages
        if type(damage) == str: damage=int(0)
    ...
    for cane in hurricanes:
        ...
        for i in damage_scale:
            if damage <= damage_scale[i]:
                values = hurricane_damages[i]
                values.append(hurricanes[cane])
                hurricane_damages[i] = values
                break
    print(hurricane_damages)

rate_by_damage(hurricane_dictionary)